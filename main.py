...
if __name__ == "__main__":
    ...
    writer = RosbagWriter(
        config["rosbag"]["output_bag_paht"],
        config["rosbag"]["output_save_compressed_image"],
        config["rosbag"]["output_storage_id"],
    )
    ...
    for i, (msg, is_image) in enumerate(reader):
        if not is_image:
            ...
            valid_ids = []
            invalid_ids = []
            for index, (xyxy, mask, confidence, class_id, _, _) in enumerate(
                detections
            ):
                if CLASSES[class_id] in DETECTION_CLASSES:
                    ...
                    if (
                        ...
                        > config["openclip"]["score_threshold"]
                    ):
                    ...
                    # or greater than 0.4.
                    if bbox_check(
                        xyxy, class_id, detections, IOU_THRESHOLD, CLASSES, CLASS_MAP
                    ) and (
